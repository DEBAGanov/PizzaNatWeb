...
**Период**: 1-2 недели
**Статус**: В процессе
**Прогресс**: 30% (3 из 10 задач завершено)
...
- **Шаги выполнения**:
  ...
  - [ ] Анализ логов браузера для определения рабочего варианта
...
- **Анализ тестового скрипта**:
  - Backend использует POST к `/auth/telegram/init` с `{"deviceId":"test_telegram_<timestamp>"}`
  - Ответ должен содержать `authToken` и `telegramBotUrl`
  - После перехода в Telegram и команды `/start` происходит авторизация через бота
- **Возможные причины**:
  - Отсутствует обязательный параметр `deviceId`
  - Неправильная структура запроса
  - Проблемы с CORS или прокси
